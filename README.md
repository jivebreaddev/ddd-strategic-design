...
## 모델링
1. `사장님`이 `메뉴`를 생성한다.
2. `사장님`이 `주문 테이블`을 생성한다.
![](1번요구사항.png)
   1. `사장님`이 `메뉴 카테고리`를 생성할 수 있다.
      - 생성 시 `메뉴 카테고리`의 `메뉴 카테고리 이름`은 비어있을 수 없다.
   2. `사장님`이 `메뉴 카테고리`들을 조회할 수 있다.
   3. `사장님`이 `상품`을 생성할 수 있다.
      - `비속어`는 포함 될 수 없다.
      - `가격`은 양수이어야 한다.
   4. `사장님`이 `상품`들을 조회할 수 있다.
   5. 사장님이 `상품 가격`을 변경할 수 있다.
      - `상품 가격`은 양수이어야 한다.
      - `메뉴 상품`들의 가격의 총합이 `메뉴`의 가격보다 높으면 `메뉴`가 `전시상태`가 `미노출` 로 바뀐다.
   6. 사장님이 `메뉴`를 생성할 수 있다.
      - `메뉴`는 `메뉴 카테고리`를 가져야한다.
      - `메뉴`는 `상품` 1개 이상으로 구성되어야 한다.
      - `메뉴`에 등록된 `상품 개수`는 1개 이상이어야 한다.
      - `메뉴`에는 `비속어`가 들어갈 수 없다.
   7. `사장님`이 `메뉴 가격`을 변경할 수 있다.
      - `메뉴`가 `메뉴 상품`들의 `가격`의 총합보다 `가격`이 클 수 없다.
   8. `사장님`이 `메뉴`를 `전시 공개` 상태로 변경할 수 있다.
      - `메뉴`가 `메뉴 상품`들의 `가격`의 총합보다 `가격`이 클 수 없다.
      - `메뉴`가 존재하지 않으면 `전시 공개` 할 수 없다.
   9. `사장님`이 `메뉴`를 `전시 숨김 상태` 할 수 있다.
      - `메뉴`가 존재하지 않으면 숨길 수 없다.
   10. `사장님`이 `메뉴`를 조회 할 수 있다.
   
2. `손님`이 `매장내주문`을 하여 `주문`에 대한 `메뉴`를 전달 받는다.
![](2번요구사항.png)
  1. `주문 종류`: 공통 (해당 조건은 공통으로 지켜져야 한다.)
     - `주문`을 생성할 때, `대기중` 으로 `주문 상태`를 변경한다.
     - `메뉴` 내의 `주문상품`들이 삭제된 `상품`이면 안된다.
     - `메뉴`가 생성된 상태이고 `전시 공개` 되어있어야한다.
     - `메뉴 가격` 총합과 `주문`에 속한 `상품 가격` 총합이 동일해야한다.
  ...
  3.  `손님`이 을 하여 `주문`에 대한 `메뉴`를 전달 받는다.
     - 그림의 2번: `주문`을 생성할 때, `주문 상태`는 `대기중`, `주문 종류`는 `매장내주문`, `주문 테이블`은 `사용중`으로 변경한다.(B 상태)
     - 그림의 3번: `주문`을 수락할 때, `주문 상태`는 `접수`(C 상태)
     - 그림의 4번: `주문`을 `손님`에게 전달할 때, `주문 상태`는 `전달`으로 변경한다.(D 상태)
     - 그림의 5번: `주문`을 완료할 때, `주문 상태`는 `주문 완료`, `주문 테이블`은 `미사용중`으로 변경한다.(E 상태)
...
![](3번요구사항.png)
   ...
   3. `손님`이 `포장주문`을 하여 주문에 대한 메뉴를 전달 받는다.
      - 그림의 2번: `주문`을 생성할 때, `주문 상태`는 `대기중`, `주문 종류`는 `포장 주문`으로 변경한다.(B 상태)
      - 그림의 3번: `주문`을 수락할 때, `주문 상태`는 `접수`(C 상태)
      - 그림의 4번: `주문`을 `손님`에게 전달할 때, `주문 상태`는 `전달`으로 변경한다.(D 상태)
      - 그림의 5번: `주문`을 완료할 때, `주문 상태`는 `주문 완료`으로 변경한다.(E 상태)

4. `손님`이 `배달주문`을 하여 주문에 대한 메뉴를 전달 받는다.
![](4번요구사항.png)
  1. `주문 종류`: 공통 (해당 조건은 공통으로 지켜져야 한다.)
     - `주문`을 생성할 때, `대기중` 으로 `주문 상태`를 변경한다.
     - `메뉴` 내의 `주문상품`들이 삭제된 `상품`이면 안된다.
  ...
  - 그림의 2번: `주문`을 생성할 때, `주문 상태`는 `대기중`, `주문 종류`는 `배달 주문`으로 변경한다.(B 상태)
  - 그림의 3번: `주문`을 수락할 때, `주문 상태`는 `접수`(C 상태)
  - 그림의 4번: `주문`을 생성 완료할 때, `주문 상태`는 `전달`으로 변경한다.(D 상태)
  - 그림의 5번: `주문`을 `라이더`에게 전달할 때, `주문 상태`는 `배달중`으로 변경한다.(E 상태)
  - 그림의 6번: `주문`을 `손님`에게 전달할 때, `주문 상태`는 `배달완료`으로 변경한다.(F 상태)
  - 그림의 7번: `주문`을 `손님`에게 전달완료할 때, `주문 상태`는 `주문완료`으로 변경한다.(G 상태)
